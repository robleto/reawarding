...
Last updated: 2026-08-23 (Best Animated ballot UI re-enabled — moved from Dormant to Active; still has open gaps, see its row below. Also 2026-08-23: Oscar Night Readiness gained a real data layer — `ceremonies`/`award_categories`/`nominations` seeded with the full 98th slate via two new ingest scripts; the UI is still wireframe-only. Previously 2026-08-22: OG export/Share sheet gained a real trigger point and moved to Active; genre-category official-winner backfill shipped; three design-exploration wireframes from chat added to Post-launch)
...
| CSV library import (Letterboxd / IMDb / TMDB / custom) | `/import` — first-class entry path with its own promise ("bring your Letterboxd history, keep your Letterboxd account"). Findable from the footer, the user menu, `/help`, `/settings`, onboarding, and `/guides/reawarding-vs-letterboxd`; `/settings/import` is a permanent 308 to it | `src/app/import/page.tsx` (promise + metadata, server), `src/components/import/LibraryImporter.tsx` (upload → map → preview → import, client), `src/lib/csvImport.ts` (parser, format detection, column mapping, scale conversion), `src/app/settings/import/page.tsx` (308 stub), `src/app/api/import/library/route.ts` | **No paywall as of 2026-08-23** — the old 50-row `FREE_IMPORT_ROW_CAP` and the premium "unlimited import" pillar are both gone (import is activation, and a 50-film library made the paid lifetime-record product meaningless). What remains is `MAX_ROWS_PER_REQUEST = 300`, a wall-clock ceiling; the client batches any export into `IMPORT_BATCH_SIZE = 200` sequential requests with progress, so file size only affects duration. Letterboxd/IMDb/TMDB exports are detected from their headers; anything else gets a guessed mapping the user confirms or corrects (column pickers + rating-scale override + watched/watchlist toggle, which is also how IMP-2's identical watched.csv/watchlist.csv headers are now resolved). Matching precision order is tmdb_id → imdb_id → normalized title+year, then live TMDB backfill. Watchlist rows *are* inserted (IMP-2). Signed-out visitors get the pitch server-rendered plus a sign-up CTA, not a dead end |
...
## Post-launch (design exploration — not scheduled)
...
**One exception as of 2026-08-23:** Oscar Night Readiness is no longer no-code — its reference data and ingest pipeline shipped (see the row below). The screen itself is still a wireframe. The other two rows remain design-only.

| Feature | Status | Notes |
|---|---|---|
| Oscar Night Readiness | **Data layer shipped 2026-08-23 — UI still wireframed, no code** | **Reference data (done):** `ceremonies` / `award_categories` / `nominations` were empty tables from the Nov 2025 awards-API era (`docs/archive/awards-api-neon-migration-20251113.sql`) and are now seeded with the complete 98th Academy Awards slate — 1 ceremony (film year 2025, `event_date` 2026-03-15), 24 categories, 125 nominations, 25 winners (Best Live Action Short was a tie, so two winners in one category — do not assume one winner per category). Built by `scripts/fetch-oscar-nominations.mjs <ceremonyNumber>` (parses Wikipedia revision text → `scripts/data/oscar-nominations-<filmYear>.json`, refuses to write a thin parse) and `scripts/ingest-oscar-nominations.mjs <json> [--apply] [--replace]` (dry-run by default; `--apply` refuses a ceremony that already has nominations; `--replace` reloads in a transaction — that's the March workflow when winners get filled in). Category slugs come from a hardcoded alias map in the fetch script, NOT from Wikipedia headings, which drift year to year; the four slugs shared with `official_award_winners` (`best-picture`/`best-animated`/`best-documentary`/`best-international`) are reused verbatim — do not coin second names for them. `ceremonies.year` is the FILM year, not the telecast year, matching `src/lib/awardsSeason.ts`. **Known gap — read before building the UI:** only 98/125 nominations resolved to a `movies` row, and coverage is 0/5 for all three short-film categories plus 1/5 for Best Documentary, so those categories are listable but not *trackable* (no `movies` row means no `rankings.seen_it` to check off) — which is exactly what the `/oscar-tracker` landing copy promises not to drop. 26 distinct films are unmatched; the features among them (e.g. The Smashing Machine, Blue Moon, Kokuho) are likely just missing from the 4,385-row `movies` subset and fixable via `scripts/import-specific-movies.ts`, but the actual shorts may need stub `movies` rows. That call is unmade. Also note `CEREMONY_DATES` in `src/lib/awardsSeason.ts` now duplicates `ceremonies.event_date` — the countdown should read the DB and let the hand-typed constant (including its `TENTATIVE` 2027 date) go. The 99th is not seeded; its nominees are announced January 2027. **Design (unchanged):** Free, current-cycle only ("current year" = the film year running Jan 1 → ceremony date, i.e. the *prior* calendar year's crop). Plain per-category "X of Y seen" list, incomplete categories sorted first, so the user never has to do the math. Ceremony countdown shown as a separate plain typographic stat next to it — deliberately **not** wrapped in a ring, since a ring there would misleadingly read as depleting alongside the days remaining rather than showing category completion. Proposed home: inside Collections (not new nav), with seasonal homepage promotion Jan 1 → ceremony date, when the current-year Awards section is otherwise thin. |
