    ...
    async function loadData() {
      ...
      try {
        ...
        const moviesWithRankings: DisplayMovie[] = movies.map((movie) => ({
          ...movie,
          // keep id as number to match DisplayMovie type
          id: movie.id,
          ranking: movie.rankings?.[0]?.ranking ?? 0,
          poster_url: movie.thumb_url,
        }));

        const groupedByYear = moviesWithRankings.reduce<Record<string, DisplayMovie[]>>((acc, movie) => {
        ...
        const formattedYears: YearData[] = Object.entries(groupedByYear).map(([year, movies]) => {
        ...
        setYearsData(formattedYears.sort((a, b) => Number(b.year) - Number(a.year))); // latest year first
      } catch (err) {
        console.error(err);
