...
function sortMovies(movies: Movie[], sortBy: SortKey, sortOrder: "asc" | "desc") {
...
function groupMovies(movies: Movie[], groupBy: GroupKey, sortBy: SortKey, sortOrder: "asc" | "desc") {
  ...
  if (groupBy === "ranking") {
    const groups: Record<string, Movie[]> = {};
    movies.forEach((movie) => {
      const ranking = movie.rankings[0]?.ranking?.toString() ?? "Unranked";
  ...
  return [{ key: "All Movies", movies: sortMovies(movies, sortBy, sortOrder) }];
}

export default function RankingsPage() {
  const [movies, setMovies] = useState<Movie[]>([]);
  const [loading, setLoading] = useState(true);const [viewMode, setViewMode] = useState<'grid' | 'list'>(() => {
    ...
    async function fetchData() {
      ...
      if (error) {
        ...
        const enriched: Movie[] = data.map((movie) => {
          const match = movie.rankings.find((r) => r.user_id === currentUserId);
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-48 gap-2">
        <svg className="animate-spin h-8 w-8 text-[#ba7a00]" viewBox="0 0 24 24" fill="none">
    ...
    <div className="max-w-screen-xl px-6 py-10 mx-auto">
      <div className="flex flex-col gap-4 mb-6 md:flex-row md:items-center md:justify-between">
        <div className="flex flex-wrap items-center gap-4 text-sm font-medium text-gray-700">
          <div className="flex items-center gap-2">
            <label htmlFor="group-select" className="whitespace-nowrap">Group by</label>
            <select
              id="group-select"
              value={groupBy}
          ...
          <div className="flex items-center gap-2">
            ...
            >
              {sortOrder === "asc" ? <span className="text-xl">▲</span> : <span className="text-xl">▼</span>}
            </button>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={`p-2 rounded-full border ${viewMode === 'list' ? 'bg-blue-100 text-blue-600 border-blue-300' : 'text-gray-400'}`}
          ...
          <button
            ...
            className={`p-2 rounded-full border ${viewMode === 'grid' ? 'bg-blue-100 text-blue-600 border-blue-300' : 'text-gray-400'}`}
            aria-label="Grid view"
          >
            <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h6v6H4V6zm0 8h6v6H4v-6zm10-8h6v6h-6V6zm0 8h6v6h-6v-6z" />
            </svg>
          </button>
        </div>
      </div>
      ...
      {grouped.map(({ key, movies }) => (
        <div key={key} className="mb-10">
          {groupBy !== 'none' && <h2 className="mb-4 text-xl font-bold text-gray-800">{key}</h2>}
          {viewMode === 'grid' ? (
