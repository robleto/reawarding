...
export default function AwardsPage() {
	...
	const formattedYears = useMemo<YearData[]>(() => {
		...
		switch (tab) {
			...
			case "best-picture":
			default:
				// Exclude clearly animated titles from Best Picture to reduce overlap perception
				moviesWithRankings = moviesWithRankings.filter((m) => !(
					hasGenre(m, "animation") || hasGenre(m, "animated")
				));
				break;
		}
	...
	}, [tab, formattedYears, totalFiltered, animationCount]);
			...
			{process.env.NODE_ENV === 'development' && (
				<div className="max-w-screen-xl mx-auto px-2 sm:px-4 mt-1">
					<div className="text-[11px] text-gray-400">
						Debug • tab: <span className="text-yellow-300">{tab}</span> • years: {formattedYears.length} • movies: {totalFiltered} • animatedExcluded: {animationCount}
					</div>
				</div>
			)}
