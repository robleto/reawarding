...
export default function AwardsPage() {
	...
	const formattedYears = useMemo<YearData[]>(() => {
		...
		switch (tab) {
			...
			case "best-picture":
			default: {
				// Conditional exclusion: prefer non-animated for Best Picture per year but keep animated if it would empty the year.
				const byYear = moviesWithRankings.reduce<Record<string, Movie[]>>((acc, m) => {
					const y = String(m.release_year);
					(acc[y] ||= []).push(m);
					return acc;
				}, {});
				const rebuilt: Movie[] = [];
				Object.values(byYear).forEach(list => {
					const nonAnimated = list.filter(m => !(hasGenre(m, "animation") || hasGenre(m, "animated")));
					rebuilt.push(...(nonAnimated.length > 0 ? nonAnimated : list));
				});
				moviesWithRankings = rebuilt;
				break;
			}
		}
	...
	const animationPresentCount = useMemo(() => {
		return formattedYears.reduce((acc, y) => acc + y.allMovies.filter(m => Array.isArray(m.genres) && m.genres.some(g => String(g ?? '').toLowerCase().includes('animation') || String(g ?? '').toLowerCase().includes('animated'))).length, 0);
	}, [formattedYears]);
	...
	useEffect(() => {
		if (process.env.NODE_ENV === 'development') {
			...
			console.log('Awards Debug', {
				...
				totalFiltered,
				animationPresentCount,
				sample,
			});
		}
	}, [tab, formattedYears, totalFiltered, animationPresentCount]);
			...
			{process.env.NODE_ENV === 'development' && (
				<div className="max-w-screen-xl mx-auto px-2 sm:px-4 mt-1">
					<div className="text-[11px] text-gray-400">
						Debug • tab: <span className="text-yellow-300">{tab}</span> • years: {formattedYears.length} • movies: {totalFiltered} • animatedPresent: {animationPresentCount}
					</div>
				</div>
			)}
