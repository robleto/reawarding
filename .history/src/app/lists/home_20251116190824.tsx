    ...
    <div className="max-w-screen-xl">
      ...
      {user && myLists.length === 0 && publicLists.length === 0 && (
        <>
          ...
          <div className="mt-6 bg-gray-900/60 border border-yellow-500/20 rounded-lg p-5">
            <div className="flex items-center justify-between">
              ...
              <a href="/lists/ready-made" className="px-3 py-2 bg-yellow-500 text-black rounded hover:bg-yellow-400">Explore</a>
            </div>
          </div>
        </>
      )}

      {/* Header with Ready‑Made CTA and Create button (logged-in users) */}
      {user && (
        <div className="flex flex-col gap-4 mb-8 md:flex-row md:items-center md:justify-between">
          <a
            href="/lists/ready-made"
            className="inline-flex items-center gap-2 px-4 py-2 text-yellow-200 bg-gray-900/60 border border-yellow-500/20 rounded-lg shadow-md hover:shadow-lg transition-all backdrop-blur"
            title="Discover auto-generated lists based on your watching"
          >
            {/* simple star icon via emoji to avoid new deps */}
            <span aria-hidden>⭐</span>
            <span>Ready‑Made Lists</span>
            {hasReadyMade && (
              <span className="ml-2 text-[10px] px-1.5 py-0.5 rounded bg-yellow-500 text-black font-semibold">New</span>
            )}
          </a>
          <button
