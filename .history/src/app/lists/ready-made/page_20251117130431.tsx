  ...
  return (
    <div className="space-y-8">
      ...
      {!user ? (
        ...
        <>
          {suggestions.length === 0 && actorSuggestions.length === 0 && genreSuggestions.length === 0 && decadeSuggestions.length === 0 && (
            <div className="bg-gray-900/60 border border-yellow-500/20 rounded-lg p-6">
          ...
          {genreSuggestions.length > 0 && (
            <div className="mt-10">
              <h2 className="text-xl font-semibold mb-3">Genre Ready‑Made Lists</h2>
          ...
          {decadeSuggestions.length > 0 && (
            <div className="mt-10">
              <h2 className="text-xl font-semibold mb-3">Decade Ready‑Made Lists</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
  ...
  if (ids.length === 0 && genre) {
    ...
    if (candidateIds.length) {
      const { data: rankRows } = await supabase
        ...
        .in('movie_id', candidateIds);
      const rows = (rankRows || []);
      const filtered = count > 100 ? rows.filter((r) => (r.ranking ?? 0) >= 9) : rows;
...
function ActorSuggestionCard({ suggestion }: { suggestion: ActorSuggestion }) {
  const name = formatActorListName(suggestion.actor, suggestion.seen_count);
  const ids = suggestion.movies.map((m) => m.id).join(',');
  const posterUrls = suggestion.movies
    .map((m) => m.poster_url)
    .filter((u): u is string => !!u)
    ...
    <div className="bg-gray-900/60 border border-yellow-500/20 rounded-lg p-5 overflow-visible">
      ...
      <div className="flex items-center justify-between">
        <div className="flex-1 pr-4">
          <h2 className="text-xl font-semibold hover:text-yellow-200 transition-colors" title={name}>{name}</h2>
          <p className="text-sm text-gray-400">Auto-generated from your seen films • Actor: {suggestion.actor}</p>
        </div>
        <div className="flex items-center gap-2">
...
function GenreSuggestionCard({ suggestion }: { suggestion: GenreSuggestion }) {
  const name = formatGenreListName(suggestion.genre, suggestion.seen_count);
  const ids = suggestion.movies.map((m) => m.id).join(',');
  const posterUrls = suggestion.movies
    .map((m) => m.poster_url)
    .filter((u): u is string => !!u)
  ...
  return (
    <div className="bg-gray-900/60 border border-yellow-500/20 rounded-lg p-5 overflow-visible">
            ...
            <div
              ...
              className="absolute w-16 h-24 rounded-xl shadow-lg border-2 border-gray-800 overflow-hidden"
                <div className="flex-1 pr-4">
                  ...
                  <p className="text-sm text-gray-400">Auto-generated from your seen films • Genre: {suggestion.genre} {suggestion.seen_count > 100 && (<span className="ml-2 text-[11px] text-yellow-300">(9–10s only)</span>)}</p>
                transform: `rotate(${(i - 2) * 7}deg)`
              }}
      ...
      <div className="flex items-center justify-between">
        <div className="flex-1 pr-4">
          <h2 className="text-xl font-semibold hover:text-yellow-200 transition-colors" title={name}>{name}</h2>
          <p className="text-sm text-gray-400">Auto-generated from your seen films • Genre: {suggestion.genre}</p>
        </div>
        <div className="flex items-center gap-2">
          <form action={dismissGenreSuggestion}>
            <input type="hidden" name="genre" value={suggestion.genre} />
...
function DecadeSuggestionCard({ suggestion }: { suggestion: DecadeSuggestion }) {
  const name = formatDecadeListName(suggestion.decade, suggestion.seen_count);
  const ids = suggestion.movies.map((m) => m.id).join(',');
  const posterUrls = suggestion.movies
    .map((m) => m.poster_url)
    .filter((u): u is string => !!u)
  ...
  return (
    <div className="bg-gray-900/60 border border-yellow-500/20 rounded-lg p-5 overflow-visible">
            ...
            <div
              ...
              className="absolute w-16 h-24 rounded-xl shadow-lg border-2 border-gray-800 overflow-hidden"
                <div className="flex-1 pr-4">
                  ...
                  <p className="text-sm text-gray-400">Auto-generated from your seen films • Decade: {suggestion.decade} {suggestion.seen_count > 100 && (<span className="ml-2 text-[11px] text-yellow-300">(9–10s only)</span>)}</p>
                transform: `rotate(${(i - 2) * 7}deg)`
      ...
      <div className="flex items-center justify-between">
        <div className="flex-1 pr-4">
          <h2 className="text-xl font-semibold hover:text-yellow-200 transition-colors" title={name}>{name}</h2>
          <p className="text-sm text-gray-400">Auto-generated from your seen films • Decade: {suggestion.decade}</p>
        </div>
        <div className="flex items-center gap-2">
          <form action={dismissDecadeSuggestion}>
            <input type="hidden" name="decade" value={suggestion.decade} />
