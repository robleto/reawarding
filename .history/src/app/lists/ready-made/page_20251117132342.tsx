...
function GenreSuggestionCard({ suggestion }: { suggestion: GenreSuggestion }) {
  ...
  const displayName = totalSeen > 100
    ? `Genre - ${suggestion.genre}: ${filteredCount}* You’ve Seen`
    : `Genre - ${suggestion.genre}: ${filteredCount} You’ve Seen`;
  const ids = suggestion.movies.map((m) => m.id).join(',');
  const posterUrls = suggestion.movies
    .map((m) => m.poster_url)
      ...
      <div className="flex items-center justify-between">
        <div className="flex-1 pr-4">
          <Link href={href} className="block">
            <h2 className="text-xl font-semibold hover:text-yellow-200 transition-colors" title={displayName}>{displayName}</h2>
          </Link>
          <p className="text-sm text-gray-400">
            Auto-generated from your seen films • Genre: {suggestion.genre}
    ...
    <div className="bg-gray-900/60 border border-yellow-500/20 rounded-lg p-5 overflow-visible">
      ...
      <div className="flex items-center justify-between">
        <div className="flex-1 pr-4">
          ...
          <p className="text-sm text-gray-400">
            Auto-generated from your seen films • Decade: {suggestion.decade}
            {suggestion.seen_count > 100 && (<span className="ml-2 text-[11px] text-yellow-300">(9–10s only)</span>)}
          </p>
        </div>
        <div className="flex items-center gap-3">
          <Link href={href} className="text-sm text-yellow-300 hover:underline whitespace-nowrap">View</Link>
