...
async function saveList(formData: FormData) {
  ...
  if (ids.length === 0 && director) {
    const { data: rows } = await supabase
      .from('rankings')
      .select('movie_id, seen_it, movie:movies(director)')
      .eq('user_id', user.id)
      .eq('seen_it', true);
    ...
    for (const r of (rows as any[] | null) || []) {
      ...
      if (dir === director) {
        ...
        if (Number.isFinite(mid)) collected.push(mid);
      }
    }
    ids = Array.from(new Set(collected));
  }
        ...
        {movies.map((m, idx) => (
          <div key={m.id} className="rounded bg-gray-900/60 border border-yellow-500/20 p-2">
            <div className="relative aspect-[2/3] rounded overflow-hidden border border-gray-800 bg-gray-800">
              <div className="absolute top-2 left-2 z-10 bg-gray-900/80 border border-gray-300/50 text-white text-lg font-unbounded font-bold px-3 py-2 rounded-md shadow-sm backdrop-blur-sm">
                {idx + 1}
              </div>
              {m.poster_url ? (
