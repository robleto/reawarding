...
async function getData(slug: string) {
  ...
  if (!user) return { user: null as const, director: null as const, count: 0, movies: [] as MovieItem[] };
  ...
  if (!matchDirector) return { user, director: null as const, count: 0, movies: [] as MovieItem[] };
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
        {movies.map((m) => (
          <div key={m.id} className="rounded bg-gray-900/60 border border-yellow-500/20 p-2">
            <div className="aspect-[2/3] rounded overflow-hidden border border-gray-800 bg-gray-800">
              {m.poster_url ? (
