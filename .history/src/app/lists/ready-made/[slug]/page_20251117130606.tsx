...
export default async function ReadyMadeDetailPage({ params }: { params: { slug: string } }) {
  const { user, category, label, count, totalCount, movies, decadeStart } = await getData(params.slug);
  ...
  const title = category === 'director'
    ? formatDirectorListName(label, count)
    : category === 'genre'
    ? formatGenreListName(label, count)
    : formatDecadeListName(label, count);
      ...
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-gray-400">
            Auto-generated from your seen films • {category === 'director' ? 'Director' : category === 'genre' ? 'Genre' : 'Decade'}: {label}
            {totalCount > 100 && <span className="ml-2 text-[11px] text-yellow-300">(showing 9–10s only • {count} of {totalCount})</span>}
          </p>
        </div>
