...
async function getSuggestions() {
  ...
  if (!user) {
    return { user: null, suggestions: [] as DirectorSuggestion[], almost: [] as AlmostDirector[], actorSuggestions: [] as ActorSuggestion[], actorAlmost: [] as AlmostActor[] };
  }
  ...
  const { data: counts, error: countErr } = await supabase
    .from('rankings')
    .select('seen_it, movie:movies(director, cast_list)')
    .eq('user_id', user.id)
    .eq('seen_it', true);
  ...
  return { user, suggestions, almost, actorSuggestions, actorAlmost };
}

export const dynamic = 'force-dynamic';

export default async function ReadyMadeListsPage() {
  const { user, suggestions, almost, actorSuggestions, actorAlmost } = await getSuggestions();
  ...
  return (
    <div className="space-y-8">
      ...
      {!user ? (
        ...
        <>
          {suggestions.length === 0 && actorSuggestions.length === 0 && (
            <div className="bg-gray-900/60 border border-yellow-500/20 rounded-lg p-6">
