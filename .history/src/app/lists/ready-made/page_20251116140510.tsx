...
async function getSuggestions() {
  ...
  if (!user) {
    return { user: null as const, suggestions: [] as DirectorSuggestion[] };
  }
  ...
  for (const row of counts || []) {
    // @ts-ignore
    const dir = row.movie?.director as string | null;
    if (!dir) continue;
  ...
  for (const [director, seen_count] of candidates) {
    ...
    const movies = (items || [])
      .map((r: any) => ({
        id: r.movie.id as number,
        title: r.movie.title as string,
        release_year: r.movie.release_year as number | null,
        poster_url: (r.movie.cached_poster_url as string | null) ?? (r.movie.poster_url as string | null),
