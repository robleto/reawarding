...
function SuggestionCard({ suggestion }: { suggestion: DirectorSuggestion }) {
  ...
  const slug = slugifyTitle(suggestion.director);
  const saveFormId = `save-${slug}`;
  const dismissFormId = `dismiss-${slug}`;
  return (
    <>
      <ReadyMadeCard
      ...
      />
      <form id={saveFormId} action={saveList} className="hidden">
      ...
      <form id={dismissFormId} action={dismissSuggestion} className="hidden">
        <input type="hidden" name="director" value={suggestion.director} />
      </form>
    </>
  );
...
function ActorSuggestionCard({ suggestion }: { suggestion: ActorSuggestion }) {
  ...
  const posterUrls = suggestion.movies
    ...
    .filter((u): u is string => !!u)
    .slice(0, 5);
  const slug = slugifyTitle(suggestion.actor);
  const saveFormId = `save-${slug}`;
  const dismissFormId = `dismiss-${slug}`;
  return (
    <>
      <ReadyMadeCard
        ...
        dismissFormId={dismissFormId}
      />
      <form id={saveFormId} action={saveActorList} className="hidden">
        <input type="hidden" name="actor" value={suggestion.actor} />
        <input type="hidden" name="count" value={suggestion.seen_count} />
        <input type="hidden" name="movie_ids" value={ids} />
      </form>
      <form id={dismissFormId} action={dismissActorSuggestion} className="hidden">
        <input type="hidden" name="actor" value={suggestion.actor} />
      </form>
    </>
  );
...
function GenreSuggestionCard({ suggestion }: { suggestion: GenreSuggestion }) {
  ...
  const slug = slugifyTitle(suggestion.genre);
  const saveFormId = `save-${slug}`;
  const dismissFormId = `dismiss-${slug}`;
  return (
    <>
      <ReadyMadeCard
        ...
        dismissFormId={dismissFormId}
      />
      <form id={saveFormId} action={saveGenreList} className="hidden">
      ...
      <form id={dismissFormId} action={dismissGenreSuggestion} className="hidden">
        <input type="hidden" name="genre" value={suggestion.genre} />
      </form>
    </>
  );
...
function DecadeSuggestionCard({ suggestion }: { suggestion: DecadeSuggestion }) {
  ...
  const slug = slugifyTitle(suggestion.decade);
  const saveFormId = `save-${slug}`;
  const dismissFormId = `dismiss-${slug}`;
  return (
    <>
      <ReadyMadeCard
        ...
        dismissFormId={dismissFormId}
      />
      <form id={saveFormId} action={saveDecadeList} className="hidden">
      ...
      <form id={dismissFormId} action={dismissDecadeSuggestion} className="hidden">
        <input type="hidden" name="decade" value={suggestion.decade} />
      </form>
    </>
  );
