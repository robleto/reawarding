    ...
    <div className="space-y-8">
      ...
      {!user ? (
        ...
        <>
          <ReadyMadeTabs
            counts={{
              all: (suggestions?.length ?? 0) + (actorSuggestions?.length ?? 0) + (genreSuggestions?.length ?? 0) + (decadeSuggestions?.length ?? 0),
              directors: suggestions?.length ?? 0,
              actors: actorSuggestions?.length ?? 0,
              genres: genreSuggestions?.length ?? 0,
              decades: decadeSuggestions?.length ?? 0,
            }}
          />
...
function DecadeSuggestionCard({ suggestion }: { suggestion: DecadeSuggestion }) {
  ...
  const filteredCount = suggestion.movies.length;
  const displayName = totalSeen > 100
    ? `Decade - ${suggestion.decade}: ${filteredCount}* You’ve Seen`
    : `Decade - ${suggestion.decade}: ${filteredCount} You’ve Seen`;
  const ids = suggestion.movies.map((m) => m.id).join(',');
  const posterUrls = suggestion.movies
    .map((m) => m.poster_url)
