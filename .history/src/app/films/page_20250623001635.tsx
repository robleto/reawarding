...
// Removed top-level useUser hook call and conditional rendering.
// The useUser hook will be called inside the RankingsPage component.
...
export default function RankingsPage() {
  ...
  useEffect(() => {
		if (!user) {
			console.warn("User not found — Supabase auth may not be available");
			return;
		}

		console.log("Current user:", user);
		console.log("Fetching for ID:", user.id);
		...
		async function fetchData() {
			...
			const { data, error } = await supabase
				.from("movies")
				.select("id, title")
				.eq("rankings.user_id", user.id);

			if (error) {
				console.error("Supabase error:", error);
			} else {
				console.log("Movies returned:", data?.length);
			}
		}
