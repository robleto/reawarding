    ...
    <div className="relative bg-gray-900/60 border border-yellow-500/20 rounded-lg h-[260px] flex flex-col overflow-visible mt-5 group transition-transform duration-200 hover:-translate-y-0.5 ring-0 hover:ring-1 hover:ring-yellow-500/20 hover:border-yellow-500/30 hover:shadow-lg">
      ...
      {posterUrls.length > 0 && (
        <div className="absolute -top-2 left-1/2 -translate-x-1/2 h-24 w-[180px] flex items-center justify-center pointer-events-none select-none z-20 transition-transform duration-200 group-hover:scale-105">
          {posterUrls.map((url: string, i: number) => (
            <div
              key={i}
              className={`absolute w-16 h-24 overflow-hidden border-2 border-gray-800 shadow-lg rounded-xl transition-transform duration-200 ease-out ${(() => {
      ...
      <div className="flex flex-col flex-1 p-6 pt-20 pb-6">
        ...
        <div className="flex items-center mt-3">
          ...
          {headerRight ? (
            <div className="ml-auto shrink-0 [&_button]:transition [&_button]:duration-150 [&_button:hover]:shadow-md [&_button:hover]:shadow-yellow-500/20 [&_button:hover]:brightness-105 [&_button:active]:scale-95">{headerRight}</div>
          ) : null}
        </div>
        {showConfirm && (
