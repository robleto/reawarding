...
export default function EditableYearSection({
  ...
  const syncView = React.useCallback(
    (view: 'default' | 'custom', override?: { nominees: Movie[]; winner: Movie | null }) => {
      if (view === 'custom') {
        const nomineesSource = override?.nominees ?? customNominees;
        const winnerSource = override?.winner ?? customWinner;
    ...
    [customNominees, customWinner, allMoviesForYear, movies, winner]
  );
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (savedIds.length > 0) {
          ...
          const shouldUseCustom = isUsingCustomView && nomineeMovies.length > 0;
  ...
  }, [year, allMoviesForYear, movies, winner, resolvedCategory, syncView, isUsingCustomView]);
  ...
  }, [user, year, resolvedCategory, loadExistingNominations]);
  ...
  }, [movies, winner, resolvedCategory, hasCustomNominations]);
            ...
            <div className="relative">
              {loadingNominations && (
                <div className="absolute inset-0 z-20 flex items-center justify-center rounded-xl bg-black/40 backdrop-blur-sm">
                  <span className="text-sm font-medium text-gray-200">Refreshing nominations…</span>
                </div>
              )}
              <div className={`flex flex-col gap-12 md:flex-row ${loadingNominations ? 'pointer-events-none opacity-60' : ''}`}>
                {/* Winner */}
