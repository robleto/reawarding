...
const ListCard = ({ list, readOnly }: ListCardProps) => {
  ...
  return (
    <Link href={`/lists/${list.id}`} tabIndex={readOnly ? -1 : 0} aria-disabled={readOnly} onClick={(e) => { if (readOnly) e.preventDefault(); }}>
  <div className={`light-glass dark:dark-glass group ${readOnly ? 'opacity-80' : ''} relative rounded-xl shadow-md h-[320px] flex flex-col overflow-visible transition-transform duration-200 hover:-translate-y-0.5 ring-0 hover:ring-1 hover:ring-yellow-500/20 hover:border-yellow-500/30 hover:shadow-lg`}>
      ...
      <span aria-hidden className="pointer-events-none absolute inset-0 rounded-xl opacity-0 group-hover:opacity-100 group-hover:animate-gold-sheen sheen-gradient z-10" />
        ...
        {posterUrls.length > 0 && (
          <PosterFanStack posterUrls={posterUrls} className="absolute -top-2 left-1/2 -translate-x-1/2" />
        )}
