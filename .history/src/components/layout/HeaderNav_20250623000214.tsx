      ...
      <div className="relative z-10 flex items-end justify-between max-w-screen-xl px-6 py-3 mx-auto">
        ...
        <div className="flex items-center gap-3">
          <div
            ...
            aria-hidden="true"
          />
          <h1 className="text-lg uppercase font-bold font-['Unbounded'] text-[#1c3728] font-inter tracking-widest">
            OscarWorthy
          </h1>
        </div>
        ...
        <nav className="hidden md:block">
          <ul className="flex items-end gap-10 text-sm font-semibold font-inter">
            {navItems.map((item) => {
              const isActive =
                pathname === item.match ||
                (item.match === '/' && pathname === '');
