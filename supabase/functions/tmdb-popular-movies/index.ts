// Fetch TMDB popular movies and upsert into Supabase movies table.
// Uses Service Role key from env; requires CRON secret for non-local calls.

async function logImport(tmdbId: number, status: string, notes?: string) {
  try {
    const supabaseUrl = Deno.env.get("SUPABASE_URL");
    const supabaseKey = Deno.env.get("SUPABASE_SERVICE_ROLE_KEY");
    if (!supabaseUrl || !supabaseKey) return;
    await fetch(`${supabaseUrl}/rest/v1/imports`, {
      method: "POST",
      headers: {
      ...
      body: JSON.stringify({ tmdb_id: tmdbId, status, notes: notes || null }),
    });
  } catch (_) {}
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const pages = Math.max(1, Math.min(3, parseInt(url.searchParams.get("pages") || "1", 10))); // cap to 3 pages/run
    const region = url.searchParams.get("region") || "US";

    let processed = 0;
    for (let p = 0; p < pages; p++) {
      const page = pageParam + p;
      const tmdbUrl = `https://api.themoviedb.org/3/movie/popular?api_key=${tmdbApiKey}&region=${region}&page=${page}`;
      ...
      for (const item of results) {
        try {
          ...
          await logImport(item.id, "success");
          processed++;
        } catch (e) {
          await logImport(item.id, "error", e instanceof Error ? e.message : String(e));
        }
