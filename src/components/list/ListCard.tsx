...
const ListCard = ({ list, readOnly }: ListCardProps) => {
  ...
  const hasPoster = posterUrls.length > 0;
  ...
  return (
    <Link href={`/lists/${list.id}`} tabIndex={readOnly ? -1 : 0} aria-disabled={readOnly}>
      {/*
        ...
        Fan posters sit at absolute top-0 within that space, visually floating above
        the glass card border without needing the parent container to allow overflow.
        This sidesteps the overflow-x-auto clipping issue on the scroll row.
      */}
      <div className={`relative ${hasPoster ? "pt-14" : ""} ${readOnly ? "opacity-80 pointer-events-none" : ""}`}>
